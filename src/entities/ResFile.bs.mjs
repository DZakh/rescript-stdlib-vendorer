...
function make(content, path) {
  return {
          content: content,
          path: path,
          moduleName: Stdlib.$$Option.getExnWithMessage(ModuleName.fromPath(path), "A ResFile should always have a valid module name.")
        };
}
...
function lint(resFile, lintContext, prohibitedModuleNames, stdlibModuleName, ignoreIssuesBeforeStdlibOpen) {
  if (resFile.moduleName === stdlibModuleName || ModuleName.isSubmodule(resFile.moduleName, stdlibModuleName)) {
    var stdlibParentDirName = Stdlib.$$Option.getExnWithMessage(resFile.path.split("/").at(-2), "A ResFile should always have a directory name");
  ...
  resFile.content.split("\n").forEach(function (line, idx) {
        ...
        if (match) {
          ...
          prohibitedModuleNames.forEach(function (prohibitedModuleName) {
                var result = Stdlib.Result.flatMap(Stdlib.Result.flatMap(Stdlib.Result.flatMap(new RegExp("^ *open " + prohibitedModuleName + "($|\\.)").test(line) ? ({
                                  TAG: "Error",
